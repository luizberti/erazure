...
// Reference Implementation: https://github.com/ronomon/reed-solomon
...
// Context layout: [w, k, m, bitmatrix...]
// Size: 3 + k * w * m * w bytes.

void crs_create(const int k, const int m, uint8_t* context) {
  assert(k >= 1 && k <= MAX_K);
  assert(m >= 1 && m <= MAX_M);
...
void crs_encode(
  ...
  reed_solomon_encode(w, k, m, bitmatrix, sources, targets, shards, shardSize);
}

void crs_xor(uint8_t* source, uint8_t* target, uint32_t length) {
  if (length > 0) dot_xor(source, target, length);
}

static void crs_search(void) {
  const int kl = 24;
  const int ks[] = {
